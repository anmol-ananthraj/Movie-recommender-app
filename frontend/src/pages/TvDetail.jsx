...
import { getTvDetails, getTvCredits, getTvExternalIds, getOmdbDetails } from "../api/tmdb"
...
function TvDetail() {
  ...
  useEffect(() => {
    async function loadShow() {
      ...
      const omdbDetails = externalIds?.imdb_id ? await getOmdbDetails(externalIds.imdb_id) : null
  ...
  if (!show) return <div className="h-screen flex items-center justify-center font-headline font-bold text-2xl">Loading...</div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[380px_1fr] gap-16 items-center w-full">
            ...
            <div className="flex flex-col gap-8">
              <div className="flex flex-wrap items-center gap-4">
                <span className="px-4 py-1.5 bg-primary/20 text-primary border border-primary/20 rounded-full text-[10px] font-black tracking-[0.2em] uppercase">FEATURED SERIES</span>
                <span className="px-4 py-1.5 bg-white/5 backdrop-blur-xl border border-white/10 text-on-surface rounded-full text-xs font-bold">{show.genres?.[0]?.name || "Drama"}</span>
                <span className="px-4 py-1.5 bg-white/5 backdrop-blur-xl border border-white/10 text-on-surface rounded-full text-xs font-bold">{show.number_of_seasons} SEASONS</span>
                <span className="px-4 py-1.5 bg-white/5 backdrop-blur-xl border border-white/10 text-on-surface rounded-full text-xs font-bold">{show.first_air_date?.slice(0, 4)}</span>
              </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
            <div className="bg-white/3 backdrop-blur-sm p-10 rounded-2xl flex flex-col gap-8 border border-white/5 hover:bg-white/5 transition-all duration-500 group relative overflow-hidden">
              ...
              <div className="relative">
                ...
                <p className="text-on-surface/40 text-xs mt-3 font-bold tracking-wide">Based on {omdbShow?.imdbVotes ? formatVoteCount(Number(omdbShow.imdbVotes.replace(/,/g, ""))) : "N/A"} audience reviews</p>
              </div>
            </div>
            ...
            <div className="bg-white/3 backdrop-blur-sm p-10 rounded-2xl flex flex-col gap-8 border border-white/5 hover:bg-white/5 transition-all duration-500 group relative overflow-hidden">
              ...
              <div className="relative">
                <div className="text-6xl font-headline font-black text-primary flex items-baseline gap-1">
                  {(omdbShow?.Ratings?.find((r) => r.Source === "Rotten Tomatoes")?.Value?.replace("%", "")) || "N/A"}<span className="text-xl text-on-surface/20 uppercase tracking-widest font-black">%</span>
                </div>
            ...
            <div className="bg-white/3 backdrop-blur-sm p-10 rounded-2xl flex flex-col gap-8 border border-white/5 hover:bg-white/5 transition-all duration-500 group relative overflow-hidden">
              ...
              <div className="relative">
                <div className="text-6xl font-headline font-black text-primary flex items-baseline gap-1">
                  {omdbShow?.tomatoUserMeter || "N/A"}<span className="text-xl text-on-surface/20 uppercase tracking-widest font-black">%</span>
                </div>
            ...
            <div className="bg-surface-container-low p-8 rounded-xl flex flex-col gap-6 border border-outline-variant/10 relative overflow-hidden group">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-on-surface/40 text-xs">r/movies â€¢ 12h ago</p>
                </div>
              </div>
            ...
            <div className="bg-surface-container-low p-8 rounded-xl flex flex-col gap-6 border border-outline-variant/10 relative overflow-hidden group">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-on-surface/40 text-xs">r/NocturnalEchoes â€¢ 5h ago</p>
                </div>
              </div>
      ...
      <section className="py-32 px-8 max-w-screen-2xl mx-auto">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-16 border-t border-white/5 pt-24">
          <div className="flex flex-col gap-6">
            ...
            <p className="text-2xl font-headline font-bold text-on-surface">{creator || "Not Listed"}</p>
          </div>
          <div className="flex flex-col gap-6">
      ...
      <footer className="w-full py-24 border-t border-white/5 bg-surface dark:bg-[#131313]">
        <div className="max-w-screen-2xl mx-auto flex flex-col items-center gap-12 px-8">
          ...
          <p className="font-body text-[10px] font-bold uppercase tracking-[0.3em] text-on-surface/20 text-center">
            Â© 2026 Reelboxd. The Digital Projectionist.
          </p>
        </div>
      </footer>
    </div>
